...
export function City({ showModal }: modalProps) {
    ...
    const toggleComponenteNewCity = () => {
        setShowNewCity(!showNewCity);
        document.body.style.overflow = 'hidden';
    };
        ...
        <section className="fixed z-[50] top-0 h-screen flex items-center w-full left-0 justify-center before:content-[''] before:bg-black before:absolute before:w-full before:h-screen before:top-0 before:left-0 before:opacity-20 before:z-0">
            <Card className="w-[35%] flex flex-col items-start">
                <div className="flex justify-between h-5 " style={{ color: 'var(--colorNeutralForegroundInverted)' }}>
                    <h3 className="px-[15px] py-[5px] top-0 absolute font-bold w-full left-0" style={{ backgroundColor: 'var(--colorNeutralForeground1)', }}>Ayuda de municipios</h3>
                ...
                <div className="flex gap-[5px]  mb-[10px]  px-[5px]">
                    <Field className="" >
                        <SearchBox className="w-[205px]" placeholder="Buscar municipio" appearance="underline" />
                    </Field>
                </div>
                ...
                <div className="w-full flex justify-end gap-[5px]">
                    <button onClick={toggleComponenteNewCity} className="flex gap-[5px] font-semibold items-center px-[7px] text-white butom rounded-[2px] py-[5px] text-[12px]">Nueva ciudad<FontAwesomeIcon icon={faPlus}></FontAwesomeIcon></button>
            ...
            {showNewCity ? <NewCity showModal={handleCloseNewCity}></NewCity> : ""}
        </section>
    )
}
